...
public:
  TestOutcome elementMetadata()
  {
    ...
    status *= (element.name() == "LagrangeQuadQ1");
    status *= (element.dim() == LagrangeQuadQ1::spatial_dim);
    status *= (element.numNodes() == LagrangeQuadQ1::nnodes);
    status *= (element.numDofsPerElement() == LagrangeQuadQ1::ndofs);
    status *= (element.order() == LagrangeQuadQ1::degree);
    status *= (element.referenceElement() == ReferenceElement::Quadrilateral);
  ...
  TestOutcome shapeAtReferenceNodes()
  {
    ...
    for (index_type node = 0; node < LagrangeQuadQ1::nnodes; ++node)
    {
      ...
      element.calcShape(nodes[node], VectorView<real_type>(values.data(), values.size()));
  ...
  TestOutcome shapePartitionOfUnity()
  {
    ...
    {
      std::array<real_type, LagrangeQuadQ1::nnodes> values = {};
      element.calcShape(point, VectorView<real_type>(values.data(), values.size()));
