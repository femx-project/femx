...
{
public:
  /** @brief Create a finite element space on a mesh with the given component count. */
  FESpace(const Mesh*          mesh,
          const FiniteElement* finite_element,
          index_type           components = 1);

  /** @brief Build the element-to-global-dof map for the space. */
  void setup();
  ...
  /** @brief Fill the global dof indices used by one element. */
  void elemDofs(index_type               ic,
                std::vector<index_type>& dofs) const;

  /** @brief Return the global dof indices used by one element. */
