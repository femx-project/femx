...
void createParentDirectory(const string& file_name)
{
  const filesystem::path path(file_name);
                                ...
                                Index num_states)
{
  ...
  {
    filesystem::create_directories(field_dir_);
    field_out_ = make_unique<FieldOutput>(space_->mesh());
  }
  if (diagnosticsEnabled())
  {
    createParentDirectory(diag_file_name_);
