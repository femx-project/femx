...
#define FEMX_MAKE_OBS_APP_NAME "make-obs-petsc"
#endif
...
void requirePetsc(const SolverParams& solver)
{
  if (solver.type == "resolve")
  {
    throw std::runtime_error(
        std::string(FEMX_MAKE_OBS_APP_NAME)
        + " requires forward.solver.type='auto' or 'petsc'");
  }
}
...
CellRange localCellRange(Index num_cells)
{
  ...
  const Index rank_id = static_cast<Index>(rank);
  const Index n_ranks = static_cast<Index>(size);
  const Index base    = num_cells / n_ranks;
  const Index extra   = num_cells % n_ranks;
  const Index begin   = rank_id * base + std::min(rank_id, extra);
  const Index count   = base + (rank_id < extra ? 1 : 0);
...
{
public:
  explicit ProgressPrinter(bool enabled = true)
    : enabled_(enabled)
  {
  }
...
void writeObservationOutputs(const make_obs::Params&            prm,
                             const MixedFESpace&                space,
                             const TimeStateTrajectory&         tr,
                             const BoundaryValues&              bc_values,
                             const TimeMatrixLinearStateSolver& state_solver)
{
  ProgressPrinter progress;
  progress.beginPhase("sample observations");
  auto obs = makeObs(space,
                     prm.obs,
                     prm.forward.time.steps,
                     state_solver.numStates(),
                     state_solver.numParams());

  TimeObservationData sampled = sampleTimeObs(*obs, tr, bc_values.prm);
  setObsLayout(sampled, space, prm.obs);
  sampled = interpolateTimeObs(sampled, prm.time, prm.forward.time.dt);
...
int run(const make_obs::Params& app_prm)
{
  PetscMPIInt rank = 0;
  PetscMPIInt size = 1;
  MPI_Comm_rank(PETSC_COMM_WORLD, &rank);
  ...
  const ForwardParams& forward = app_prm.forward;
  const Index          steps   = forward.time.steps;
  requirePetsc(forward.solver);
  ...
  if (rank == 0)
  {
    std::cout << FEMX_MAKE_OBS_APP_NAME << ": ranks = " << size
              << ", dofs = " << space.numDofs()
  ...
  const CellRange      cells = localCellRange(space.numElems());
  ns_eq.setCellRange(cells.begin, cells.end);
  ...
  KspLinearSolver lin_solver(PETSC_COMM_WORLD);
  setKspDefaults(lin_solver);
  ...
  ProgressPrinter progress(rank == 0);
  ...
  if (rank == 0)
  {
    ...
    writeObservationOutputs(app_prm, space, tr, bc_values, state_solver);
  }
