...
void checkFieldSize(const Mesh& mesh, const HostVector<Real>& vals)
{
...
void writeDoubleDataset(hid_t                      file,
                        ...
                        const HostVector<Real>&    data,
                        const HostVector<hsize_t>& dims)
{
  hid_t dspace =
...
{
  HostVector<Real> data(vals.size());
...
void writeVectorDataset(hid_t                                  file,
                        const std::string&                     path,
                        const std::array<HostVector<Real>, 3>& vals)
{
  const Index      num_nodes = vals[0].size();
  HostVector<Real> data(num_nodes * 3);
...
void writeHdf5(const std::string&                         fname,
               const Mesh&                                mesh,
               const HostVector<TimeSeriesDataOut::Step>& steps)
{
  hid_t file =
...
void TimeSeriesDataOut::checkReady() const
{
  ...
  for (const auto& step : steps_)
  {
    ...
    {
      for (const HostVector<Real>& comp : vec.vals)
      {
