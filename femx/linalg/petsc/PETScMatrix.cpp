...
Index PETScMatrix::numRows() const
{
  return rows_;
}

Index PETScMatrix::numCols() const
{
  return cols_;
}
...
Mat PETScMatrix::mat() const
{
  if (mat_ == nullptr)
  {
    throw std::runtime_error("PETScMatrix is not initialized");
  }
  return mat_;
}
...
void PETScMatrix::setDefaultNonzerosPerRow(Index count)
{
  if (count <= 0)
  {
    throw std::runtime_error("PETScMatrix preallocation must be positive");
  }
...
void PETScMatrix::resize(const CsrPattern& pettern, const PETScVector& lyt)
{
  checkInitialized();
  ...
  if (lyt.size() != pettern.rows())
  {
    throw std::runtime_error(
        "PETScMatrix row layout does not match sparsity pattern");
  }
...
void PETScMatrix::set(Index row, Index col, Real value)
{
  setValue(row, col, value, INSERT_VALUES);
}

void PETScMatrix::add(Index row, Index col, Real value)
{
  setValue(row, col, value, ADD_VALUES);
}

void PETScMatrix::addAtomic(Index row, Index col, Real value)
{
...
void PETScMatrix::addBlock(const PetscInt*    dofs,
                           Index              nd,
...
void PETScMatrix::addBlock(const PetscInt* dofs, Index nd, const Real* vals)
{
...
void PETScMatrix::zeroRowsColumns(const Vector<Index>& rows,
                                  Real                 diagonal,
                                  ...
                                  PETScVector&         rhs)
{
  ...
  {
    throw std::runtime_error(
        "PETScMatrix zeroRowsColumns received incompatible vectors");
  }
  if (rows.empty())
  {
    return;
  ...
  for (Index row : rows)
  {
    ...
    {
      throw std::runtime_error(
          "PETScMatrix zeroRowsColumns row is out of range");
    }
...
void PETScMatrix::zeroRows(const Vector<Index>& rows, Real diagonal)
{
  if (rows.empty())
  {
    return;
  ...
  for (Index row : rows)
  {
    if (row < 0 || row >= rows_)
    {
      throw std::runtime_error("PETScMatrix zeroRows row is out of range");
    }
    petsc_rows.push_back(static_cast<PetscInt>(row));
  }
...
void PETScMatrix::apply(const Vector<Real>& dir, Vector<Real>& out) const
{
  if (dir.size() != numCols())
  {
    throw std::runtime_error(
...
void PETScMatrix::applyT(const Vector<Real>& dir, Vector<Real>& out) const
{
  if (dir.size() != numRows())
  {
    throw std::runtime_error(
...
void PETScMatrix::setValue(Index row, Index col, Real value, InsertMode mode)
{
  if (mat_ == nullptr)
  {
    throw std::runtime_error("PETScMatrix is not initialized");
...
void PETScMatrix::checkInitialized()
{
  PetscBool initialized = PETSC_FALSE;
  check(PetscInitialized(&initialized), "PetscInitialized");
  if (initialized != PETSC_TRUE)
  {
    throw std::runtime_error("PETScMatrix requires initialized PETSc");
  }
}

void PETScMatrix::check(PetscErrorCode ierr, const char* operation)
{
...
void PETScMatrix::checkMPI(int ierr, const char* operation)
{
  if (ierr != MPI_SUCCESS)
  {
    throw std::runtime_error(std::string(operation) + " failed");
...
void PETScMatrix::computePreallocation(const CsrPattern& pettern,
                                       PetscInt          begin,
                                       PetscInt          end,
                                       Vector<PetscInt>& d_nnz,
                                       Vector<PetscInt>& o_nnz)
