...
public:
  ...
  PetscErrorCode solve(const Vector<Real>& init, TaoResult& result)
  {
    ...
    try
    {
      ...
      PetscCall(::femx::algebra::detail::copyToPETSc(init, prm.get()));

      TaoReducedFunctionalAdapter adapter(num_params_, value_grad_);
      ...
      if (has_bounds_)
      {
        ...
        PetscCall(::femx::algebra::detail::copyToPETSc(bounds_.lower,
                                                      lower.get()));
        PetscCall(::femx::algebra::detail::copyToPETSc(bounds_.upper,
                                                      upper.get()));
...
private:
  static PetscErrorCode monitorCallback(Tao tao, void* ctx)
  {
    ...
    Vector<Real> current;
    PetscCall(::femx::algebra::detail::copyFromPETSc(prm, current));
